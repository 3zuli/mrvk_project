...
#include "../include/kv01_driver/common_functions.h"
...
//            ROS_INFO("%x", message[i]);
    ...
    if (poloha_mimo[speed->joint] == -1 && speed->speed < 0) {
        ...
        return;
               ...
               poloha_mimo[speed->joint] == 1 && speed->speed < 0) {
...
void *position_control_thread(void *node) {
    ...
    while (ros::ok()) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (((speed[i] > 0) && (real_position_in_fun[i] > (kv01_fun->machine_data.Ulimit_j[i] - 0.17))) ||
                ((speed[i] < 0) && (real_position_in_fun[i] < (kv01_fun->machine_data.Dlimit_j[i] + 0.17)))) {
            ...
            if (flag_position_setup[i]) {
                ...
                if ((((speed[i] > 0) && (real_position_in_fun[i] > position[i])) ||
                     ((speed[i] < 0) && (real_position_in_fun[i] < position[i])) || speed[i] == 0)) {
                    joint_speed.joint = i;
                    joint_speed.command = comm_motory_speed;
                    joint_speed.speed = 0;
                    speed_pub.publish(joint_speed);
...
void *joint_states_fun_thread(void *node) {
    ros::Rate loop_rate(
            50);//finalna frekvencia komunikacie cela vzorka prida raz za 80 ms pricom menic komunikuje s pc za cca 13 ms
...
int main(int argc, char **argv) {
    ...
    n.getParam("baudrate", baud);
    ROS_INFO("KV01 driver: Using port %s @ %d baud", port.c_str(), baud);
